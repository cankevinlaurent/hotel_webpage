<!--
	var strvalue="          Èç¹ûÄãÏ²»¶ÄÏ¾©µ¤·ï¹ú¼ÊÇàÄêÂÃ¹Ý£¬±ðÍüÁË¸æËßÄãµÄÅóÓÑÅ¶£¡Ð»Ð»ÄãµÄÖ§³Ö:)          ";

	function init()
	{
		if (top.location !== self.location) top.location=self.location;
		setInterval("movestatus()",300);
		adShow();
		var pos = window.location.href.indexOf("?");
		var parastr = window.location.href.substring(pos+1);
		if (pos>0) {
			pos = parastr.indexOf("=");
			var para = parastr.substring(pos+1);
			slide(para);
		}
		else slide(0);
	}
...
------------------------------------------------------------ */
	function slide(num)
	{
		for(var i=0, j=-1;i<document.getElementById("title"+num).parentNode.childNodes.length;i++)
		{
			if(document.getElementById("title"+num).parentNode.childNodes[i].nodeName!="#text")
			{
				j++;
				document.getElementById("title"+j).style.color="#FC0";
				document.getElementById("title"+j).style.background="#322A1E";
				document.getElementById("slide"+j).style.display="none";
			}
		}
		document.getElementById("title"+num).style.color="#00FF00";
		document.getElementById("title"+num).style.background="#534531";
...
----------------------------------------------------------------------------*/
	function movestatus(){
		strvalue=strvalue.substring(1,strvalue.length)+strvalue.substring(0,1);
...
/* ±¾ÌØÐ§µÄÈë¿ÚµØÖ·£¬¿É·ÅÈëbody onloadÖÐ */
...
*/
		...
		var CB_Preload='be';
		var CB_TextNav='be';
		var CB_NavTextPrv='ÉÏÒ»Ò³';
		var CB_NavTextNxt='ÏÂÒ»Ò³';
		var CB_NavTextCls='¹Ø±Õ';
...
toÎªobject¶ÔÏó 
tÎª¾àÀë¶¥¶Ë¾àÀë 
µ±c   =   0µÄÊ±ºò£¬aÎª¾àÀë×ó±ßµÄ¾àÀë 
µ±c   !=   0µÄÊ±ºò£¬aÎª¾àÀëÓÒ±ßµÄ¾àÀë 
loÎª¹Ø±Õ°´Å¥ 
*/ 
...
---------------------------------------------------------------*/
//slide
var scontainer,scHeight,listH,sint,sinum=0,sdir=1,cnum,cstop,step,isinit,txts,txtint;

/* ±¾ÌØÐ§µÄÈë¿ÚµØÖ·£¬¿É·ÅÈëbody onloadÖÐ */
function clickslide() {
 scontainer=getid("inner_slide");
 scHeight=scontainer.offsetHeight;
 listH=scontainer.getElementsByTagName("ul")[0].offsetHeight;
 for(var i=0;i<scontainer.getElementsByTagName("li").length;i++){
scontainer.getElementsByTagName("li")[i].getElementsByTagName("img")[0].index=i+1;
scontainer.getElementsByTagName("li")[i].getElementsByTagName("img")[0].onclick=function(){showp(this.index)}
}
 }

function inner_slide(n){
 if(n==1){sdir=(-1)}else{sdir=1} 
 if(scontainer.scrollTop<=0 && n==1){return false;}else if(scontainer.scrollTop>=(listH-scHeight) && n==2){return false;}
 sinum=0;
 step=6;
 cstop=scontainer.scrollTop;
 sint=setInterval(inner_mov,2);
    return true;
 }
function inner_mov(){
 scontainer.scrollTop+=(step*sdir);
 if(step>2){ step-=1}
 sinum+=step;
 if(sinum>=82){clearInterval(sint);scontainer.scrollTop=cstop+82*sdir;showp(scontainer.scrollTop/82+1)}
 if(scontainer.scrollTop<=0){getid("sbutton1").style.visibility="hidden";}else{getid("sbutton1").style.visibility="visible";}
 if(scontainer.scrollTop>=(listH-scHeight)){getid("sbutton2").style.display="none";}else{getid("sbutton2").style.display="block";}
 }
function txtm(){
if(txts>0){txts--;}else{clearInterval(txtint)}
getid("inner_slide_txt").style.marginTop=txts+"px";
}
function showp(n){
 getid("photo").src=scontainer.getElementsByTagName("li")[n-1].getElementsByTagName("img")[0].getAttribute("src");
 getid("inner_slide_txt").innerHTML=scontainer.getElementsByTagName("li")[n-1].getElementsByTagName("img")[0].getAttribute("alt");
 txts=20; 
 clearInterval(txtint);
 txtint=setInterval("txtm()",10);
 for(var i=0;i<scontainer.getElementsByTagName("li").length;i++){scontainer.getElementsByTagName("li")[i].getElementsByTagName("img")[0].className="inner_slide_lightoff";}
 scontainer.getElementsByTagName("li")[n-1].getElementsByTagName("img")[0].className="inner_slide_lighton";
 }
